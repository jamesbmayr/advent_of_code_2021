...
def get_boundary(segment):
	...
	# flip x and y for pretty print
	return {"startX": startY, "endX": endY, "startY": startX, "endY": endX}



def update_grid(grid, boundary):
	"""marks cells within boundary on grid"""

	x = boundary["startX"]
	while x <= boundary["endX"]:
		y = boundary["startY"]
		while y <= boundary["endY"]:
			grid[x][y] = grid[x][y] + 1
			y = y + 1
		x = x + 1
...
# print(get_danger_points(data, 2))



# part 2

def get_danger_points_with_diagonals(segments, threshold):
	"""create a grid and mark the number of times each cell is part of a line segment, including diagonals"""

	grid = [[0 for x in range(1000)] for y in range(1000)]

	for segment in segments:
		boundary = get_boundary_with_diagonals(segment)
		if boundary:
			grid = update_grid_with_diagonals(grid, boundary)	
...
def get_boundary_with_diagonals(segment):
	"""identifies boundaries to mark, including diagonals"""
	...
	# flip x and y for pretty print
	return {"x1": y1, "y1": x1, "x2": y2, "y2": x2, "startX": startY, "endX": endY, "startY": startX, "endY": endX, "slope": slope}



def update_grid_with_diagonals(grid, boundary):
	"""marks cells within boundary on grid, including diagonals"""

	if boundary["slope"] == 0 or boundary["slope"] == "Infinity":
		x = boundary["startX"]
		while x <= boundary["endX"]:
			y = boundary["startY"]
			while y <= boundary["endY"]:
				...
				y = y + 1
			x = x + 1
	elif boundary["slope"] == 1:
		constant = boundary["x1"] - boundary["y1"]
		x = boundary["startX"]
		while x <= boundary["endX"]:
			y = boundary["startY"]
			while y <= boundary["endY"]:
		...
		print("error! invalid slope:", boundary["slope"])
	return grid



# print(get_danger_points_with_diagonals(data, 2))
